...
import NavbarDelta from '@/components/nav_delta';
import FooterDelta from '@/components/footer_delta';
...
export default function ReportGeneratorPage() {
    ...
    const [selectedTemplateId, setSelectedTemplateId] = useState<string | null>('comprehensive-energy-analysis');
    const [template, setTemplate] = useState(reportTemplates[0].template); // Default to first template
    const [generatedContent, setGeneratedContent] = useState<GeneratedSection[] | null>(null);
    ...
    const handleGenerate = async () => {
        ...
        try {
            console.log('[Report Gen] Sending request with:', { inputTextLength: inputText.length, templateLength: template.length });
            ...
            console.log('[Report Gen] Response status:', response.status, response.statusText);

            if (!response.ok) {
                const errorData = await response.json().catch(() => ({ error: 'Unknown error' }));
                console.error('[Report Gen] API error:', errorData);
                throw new Error(errorData.error || '生成報告失敗');
            }

            const data = await response.json();
            console.log('[Report Gen] Received data:', data);
            ...
            console.log('[Report Gen] Success! Generated', data.sections.length, 'sections');
        } catch (error) {
    ...
    return (
        <>
            <NavbarDelta />
            <div className="min-h-screen bg-gradient-to-br from-slate-100 via-slate-50 to-blue-50 dark:from-slate-950 dark:via-slate-900 dark:to-blue-950 pt-20">
                {/* Header */}
